<?php
/**
 * Surcharge des fonctions de calcul des prix par Bigaille
 *
 * Modification des règles pour les prix, on va cherche le prix de l'objet lié `prix_objet` :
 * avant : prix_{objet}.php > colonne 'prix'
 * après : prix_{objet}.php > colonne 'prix' de prix_objet > colonne 'prix'
 *
 * @plugin		 Bigaille
 * @copyright	2015
 * @author		 C.R
...
function inc_prix_ht_dist($type_objet, $id_objet, $arrondi=2){
	...
	if ($type_objet
		...
		// S'il n'y a pas de fonction, regardons s'il y a un prix_objet lié
		elseif ($prix_objet_ht = sql_getfetsel('prix_ht', 'spip_prix_objets', 'objet='.sql_quote($type_objet).' AND id_objet='.intval($id_objet))){
			$prix_ht = $prix_objet_ht;
		}
		// Sinon, regardons s'il existe des champs normalisés, ce qui évite d'écrire une fonction pour rien
...
function inc_prix_dist($type_objet, $id_objet, $arrondi=2){
	...
	// Sinon on regarde s'il y a un prix_obet lié
	elseif ($prix_objet = sql_getfetsel('prix', 'spip_prix_objets', 'objet='.sql_quote($type_objet).' AND id_objet='.intval($id_objet))){
		$prix = $prix_objet;
	}
